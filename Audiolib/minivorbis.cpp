...
// Don't remove this. Enable Alib support by passing -DALIB_OGG_SUPPORT.
// Check qmake file.
...
static int start_decoder(vorb *f)
{
   ...
   //////////////////////////////////////////////
   // BEGIN COMMENT READING
   int prev_loc = f->f->tell(); // HACK

   if (get8(f) != VORBIS_packet_comment)            return error(f, VORBIS_invalid_comment);
   for (i=0; i < 6; ++i) header[i] = get8(f);
   if (!vorbis_validate(header))                    return error(f, VORBIS_invalid_comment);

   // get vendor string
   int vendor_string_len = get32(f);
   f->vendor_string = (char *)malloc(vendor_string_len + 1);
   memset(f->vendor_string, 0, vendor_string_len + 1);
   getn(f, (uint8*)f->vendor_string, vendor_string_len);

   // get comments
   f->num_comments = get32(f);
   f->comments = (char **)malloc(sizeof(char *) * f->num_comments);
   memset(f->comments, 0, sizeof(char *) * f->num_comments);

   for(i=0; i < f->num_comments; ++i) {
       unsigned int comment_len = get32(f);
       f->comments[i] = (char *) malloc(comment_len + 1);
       memset(f->comments[i], 0, comment_len + 1);
...
static void vorbis_deinit(stb_vorbis *p)
{
   ...
   // comments are always dynamically allocated
   free(p->vendor_string);
   for(i=0; i < p->num_comments; ++i)
       free(p->comments[i]);
   free(p->comments);
}
...
char *stb_vorbis_get_comment_by_field(stb_vorbis *f, const char *field_name, const char *default_value)
{
    ...
    for(i=0; i < f->num_comments; ++i) {
        char *comment = f->comments[i];
        j = strchr(comment, '=') - comment;
        if(strncmp(comment, field_name, j) == 0)
            return comment + j + 1;
    }
...
stb_vorbis * stb_vorbis_open_file_section(alib::BinaryStream *file, int close_on_free, int *error, stb_vorbis_alloc *alloc, unsigned int length)
{
   stb_vorbis *f, p;
   vorbis_init(&p, alloc);
   p.f = file;
...
stb_vorbis * stb_vorbis_open_file(alib::BinaryStream *file, int close_on_free, int *error, stb_vorbis_alloc *alloc)
{
