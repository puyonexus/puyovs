...
void ChatroomForm::statusMessage(ChatroomForm::StatusEvent type, QString arg)
{
    QString message;

    switch(type)
    {
    case ChannelJoinedEvent: message = tr("Chatroom %s joined.", "Messages:ChannelJoined"); break;
    case ChannelLeftEvent: message = tr("Chatroom %s left.", "Messages:ChannelLeft"); break;
    case PeerJoinedEvent: message = tr("%s joined chatroom.", "Messages:PeerJoined"); break;
    case PeerLeftEvent: message = tr("%s left chatroom.", "Messages:PeerLeft"); break;
    case MatchCancelEvent: message = tr("Matchmaking canceled.", "Messages:MatchCancel"); break;
    case MatchLeftEvent: message = tr("Challenger left.", "Messages:MatchLeft"); break;
    case MatchAutorejectEvent: message = tr("User autorejected.", "Messages:MatchAutoreject"); break;
    case MatchAutorejectSelfEvent: message = tr("Autorejected a challenge.", "Messages:MatchAutorejectSelf"); break;
    case MatchDeclineEvent: message = tr("User declined the challenge.", "Messages:MatchDecline"); break;
    case MatchAcceptEvent: message = tr("Get ready to play...", "Messages:MatchAccept"); break;
    }
...
void ChatroomForm::appendLine(QString html)
{
    ...
    /*ui->ChatTextEdit->insertHtml(QString("<font color='gray'>[%1]</font> ").arg(now.toString("hh:mm")));
    ui->ChatTextEdit->insertHtml(html);
    ui->ChatTextEdit->insertHtml("<br/>\r\n");*/
    ui->ChatTextEdit->moveCursor(QTextCursor::End);
}

void ChatroomForm::updateChallengeButton()
{
    QListWidgetItem *current = ui->UserListWidget->currentItem();

    if(current && current->text() != mProxy->nick() && current->data(Qt::UserRole) == 0 &&
       !challenge.beingChallenged && challenge.challengedUser.isEmpty() && !isAutoRejectEnabled())
...
bool ChatroomForm::isAutoRejectEnabled()
{
    return ui->AutoRejectCheckBox->isChecked();
}

ppvs::rulesetInfo_t ChatroomForm::createRules()
{
    ppvs::rulesetInfo_t rs = gameManager->createRules();
    /*
    rs.Nplayers = ui->PlayersSpinbox->value();

    switch(ui->RulesComboBox->currentIndex())
    {
    case 0: rs.rulesetType = TSU_ONLINE; break;
    case 1: rs.rulesetType = FEVER_ONLINE; break;
    case 2: rs.rulesetType = FEVER15_ONLINE; break;
    case 3: rs.rulesetType = ENDLESSFEVERVS_ONLINE; break;
    }
    */
...
void ChatroomForm::userInfoReceived(QString msg)
{
...
void ChatroomForm::statusReceived(QString nick, uchar status)
{
...
void ChatroomForm::peerJoined(QString peer)
{
...
void ChatroomForm::on_AutoRejectCheckBox_toggled(bool checked)
{
    mProxy->setStatus(checked ? 1 : 0);
}

void ChatroomForm::on_UserListWidget_currentItemChanged(QListWidgetItem *current, QListWidgetItem *previous)
{
    updateChallengeButton();
    if (current)
    {
...
void ChatroomForm::getInvitation(QString invite)
{
    //read rules to determine number of players
...
void ChatroomForm::setAutoreject()
{
    Settings &settings = pvsApp->settings();
...
void ChatroomForm::loadSettings()
{
    Settings &settings = pvsApp->settings();
    ui->PlayersSpinbox->blockSignals(true);
    ui->RulesComboBox->blockSignals(true);
...
void ChatroomForm::on_UserListWidget_itemChanged(QListWidgetItem *item)
{
    if (item)
...
void ChatroomForm::on_EntryTextEdit_textChanged()
{
    int max = 240;
