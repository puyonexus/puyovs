...
bool FShaderGL::compile()
{
    ...
    ext.glShaderSource(vertexShader, 1, &vertexSource, 0);
    ext.glShaderSource(fragmentShader, 1, &fragmentSource, 0);
    ...
    {
        char log[4096] = {0};
        ext.glGetShaderInfoLog(vertexShader, sizeof(log), 0, log);
    ...
    {
        char log[4096] = {0};
        ext.glGetShaderInfoLog(fragmentShader, sizeof(log), 0, log);
    ...
    {
        char log[4096] = {0};
        ext.glGetShaderInfoLog(program, sizeof(log), 0, log);
        QMessageBox(QMessageBox::Warning, "Puyo VS", QString("GLSL Error: \n") + log, QMessageBox::Ok).exec();
