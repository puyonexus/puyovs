...
ppvs::game *GameWidget::game()
{
    return mGame;
}

ChatWindow *GameWidget::chatWindow()
{
    return mChatWindow;
}
...
void GameWidget::chatVisibilityChanged(bool visible)
{
...
bool GameWidget::event(QEvent *event)
{
    ...
    case QEvent::WindowDeactivate:
        if( (pvsApp->activeWindow() && pvsApp->activeWindow()->inherits("GameWidget"))
                || pvsApp->activeWindow()==0)
        {
            if (mGame)
                mGame->setWindowFocus(false);
        }
        //if(pvsApp->activeWindow() == 0 || !pvsApp->activeWindow()->inherits("GameWidget"))
        //    pvsApp->setMusicMode(PVSApplication::MusicPause);
        break;
    }
