    ...
    ui(new Ui::AdminDialog)
{
    ...
    connect(mClient, SIGNAL(adminResponseReceived(QString)), SLOT(adminResponseReceived(QString)));
...
void AdminDialog::adminResponseReceived(QString message)
{
...
void AdminDialog::on_SendButton_clicked()
{
    uchar subchannel;
    if (isAdminCommand())
...
bool AdminDialog::isAdminCommand()
{
    QString command=ui->CommandComboBox->lineEdit()->text();
    if (command=="newmod" || command=="newadmin" || command=="demote" || command=="delete")
        return true;
    else
        return false;
}

void AdminDialog::on_CommandComboBox_currentIndexChanged(const QString &arg1)
{
    if (arg1=="kick")
