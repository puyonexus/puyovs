    ...
    : feverGauge(g->data), m_feverLight(g->data)
{
    ...
    m_randomizerNextList = NULL;
    m_randomizerFeverChain = NULL;
    m_randomizerFeverColor = NULL;
...
{//Game code
    ...
    /*if (controls.Up==1)
    {
        data->front->musicVolume(0.1f,false);
    }*/
    //if (controls.Up==1)
    //{
        //activeField->dropField("223430434340223434432122423214232122123414234124123412000000");
        //activeField->dropField("000030000040203430122340241231412342234121123412412341123412");
    //}
    //if (m_playernum==1)
    //debugstring=to_string(currentphase);
    /*debugstring=std::string("phase: ")+to_string(currentphase)+"\n"+
                std::string("x: ")+to_string(movePuyos.getPosX1())+
                std::string(" y: ")+to_string(movePuyos.getPosY1())+
                std::string(" rot: ")+to_string(movePuyos.getRotation())+"\n"+
                std::string("fallcount: ")+to_string(movePuyos.getFallCounter()) +std::string(" rotcount: ")+to_string(movePuyos.getRotateCounter())+
                "\n"+"wait: "+to_string(waitForConfirm);
    */
...
//        for(i=m_nextList.begin();i<m_nextList.end();i++)
//        {
//            debugstring=debugstring+to_string(*i);
//        }
//
//    }
    //debugstring=to_string(activeField->count());
//    debugstring=to_string(chain);
//    debugstring=to_string(currentphase);
    //debugstring=to_string(m_nextList.size());
    //debugstring=to_string(currentgame->players[0]->debug)+" "+to_string(currentgame->players[1]->debug);
    //debugstring=to_string(m_transitionTimer);

//    if (m_playernum==1)
//        debugstring="";
//    debugstring+=to_string(chain)+" "+to_string(predictedChain)+"\n";
//
//    debugstring=debugstring+"EQ:\t"+to_string(EQ)+"\n"+
//            "nCQ:\t"+to_string(normalGarbage.CQ)+"\n"+
//            "nGQ:\t"+to_string(normalGarbage.GQ)+"\n"+
//            "fCQ:\t"+to_string(feverGarbage.CQ)+"\n"+
//            "fGQ:\t"+to_string(feverGarbage.GQ)+"\n"+
//            "size:\t"+to_string(activeGarbage->accumulator.size())+"\n\n";
//            "\t"+to_string(currentgame->players[0]->chain)+"/"+to_string(currentgame->players[0]->predictedChain)+"\t"+to_string(currentgame->players[1]->chain)+"/"+to_string(currentgame->players[1]->predictedChain)+"\n";

//    if (m_playernum==1)
//        debugstring=debugstring+"attdef: "+to_string(int(attdef))+"\n";

//    debugstring="EQ:\t"+to_string(currentgame->players[0]->EQ)+"\t"+to_string(currentgame->players[1]->EQ)+"\n"+
//            "CQ:\t"+to_string(currentgame->players[0]->activeGarbage->CQ)+"\t"+to_string(currentgame->players[1]->activeGarbage->CQ)+"\n"+
//            "GQ:\t"+to_string(currentgame->players[0]->activeGarbage->GQ)+"\t"+to_string(currentgame->players[1]->activeGarbage->GQ)+"\n"+
//            "size:\t"+to_string(currentgame->players[0]->activeGarbage->accumulator.size())+"\t"+to_string(currentgame->players[1]->activeGarbage->accumulator.size())+"\n"+
//            "\t"+to_string(currentgame->players[0]->chain)+"/"+to_string(currentgame->players[0]->predictedChain)+"\t"+to_string(currentgame->players[1]->chain)+"/"+to_string(currentgame->players[1]->predictedChain)+"\n";
}
...
//        colorMenuBorder[3].setSize(24,colorMenuBorder[0].getY()-colorMenuBorder[6].getY()-24);
