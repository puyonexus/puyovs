...
void Game::setRules()
{
	...
	{
	case ENDLESS:
		currentruleset = new EndlessRuleSet();
		break;
	case TSU:
		currentruleset = new TsuRuleSet();
		break;
	case FEVER:
		currentruleset = new FeverRuleSet();
		break;
	case ENDLESSFEVER:
		currentruleset = new EndlessFeverRuleSet();
		break;
	case ENDLESSFEVERVS:
		currentruleset = new EndlessFeverVsRuleSet();
		break;
	case TSU_ONLINE:
		currentruleset = new TsuOnlineRuleSet();
		break;
	case FEVER_ONLINE:
		currentruleset = new FeverOnlineRuleSet();
		break;
	case FEVER15_ONLINE:
		currentruleset = new Fever15OnlineRuleSet();
		break;
	case ENDLESSFEVERVS_ONLINE:
		currentruleset = new EndlessFeverVsOnlineRuleSet();
		break;
	default:
		currentruleset = new EndlessRuleSet();
		break;
...
void Game::playGame()
{
	...
	for (int i = 0; i < players.size(); i++)
	{
		...
		FeInput input = data->front->inputState(i);
...
void Game::checkEnd()
{
	...
	if (menuSelect == 0)
	{
		...
		if (data->windowFocus)
		{
			data->front->musicEvent(MusicCanStop);
		}
...
void Game::sendDescription() const
{
	...
	if (ch != nullptr && network->channelManager.getStatus(channelName, network->getPVS_Peer()) == 1)
	{
		if (!settings->rankedMatch)
		{
			...
			const RuleSetInfo* rs = &(settings->rulesetInfo);
...
void Game::loadReplay(std::string replayfile)
{
	...
	for (const auto& player : players)
	{
		...
		infile.read((char*)&rph, sizeof(ReplayPlayerHeader));
		...
		if (movSize > 0)
		{
			infile.seekg(sizePrevious + sizeof(ReplayPlayerHeader), std::ios::beg);
		...
		if (mesSize > 0)
		{
			infile.seekg(sizePrevious + sizeof(ReplayPlayerHeader) + movSizeComp, std::ios::beg);
...
void Game::loadMusic()
{
	...
	data->front->musicEvent(MusicContinue);
	targetVolumeNormal = 100;
	targetVolumeFever = 100;
