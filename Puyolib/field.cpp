...
void field::init(fieldProp properties,player* pl)
{
    ...
    //set fieldrender
    // !!!!!!!!!!!!!!
    //fieldRenderImage.set(&m_fieldSpriteFinal,global->AppWindow);
    // !!!!!!!!!!!!!!

    m_visible=true;
    m_properties=properties;
    //calculate center (does not include x,y scale)
    m_centerX=properties.gridX*properties.gridWidth/2.0;
    m_centerY=(properties.gridY-3)*properties.gridHeight;
    ...
    //create fieldrender
    //m_player->getRenderImage().create(m_properties.gridX*m_properties.gridWidth,(m_properties.gridY-3)*m_properties.gridHeight);
}
...
posVectorFloat field::getTopCoord(float offset)
...
posVectorFloat field::getBottomCoord(bool relative)
...
posVectorFloat field::getGlobalCoord(int x, int y)
...
posVectorFloat field::getLocalCoord(int x, int y)
{//convert an indexed position to field position
    posVectorFloat pv;
    pv.x=(m_properties.gridWidth*x);
    pv.y=(m_properties.gridHeight*(m_properties.gridY-3-y));
    return pv;
}

posVectorFloat field::getFieldSize()
{//returns current field size
    return posVectorFloat(m_properties.gridX*m_properties.gridWidth,
                          (m_properties.gridY-3)*m_properties.gridHeight);
}

posVectorFloat field::getFieldScale()
{
...
{//This function creates a colored puyo on the field
    ...
    fieldPuyoArray[x][y]->fallDelay=fallDelay;
    return true;
}
...
int field::dropSingle(int x, int y)
...
bool field::set(int x,int y,puyo* newpuyo)
...
void field::setLink(int x,int y,direction dir)
...
void field::unsetLink(int x,int y,direction dir)
{//Disconnect a puyo
    //invalid position
    if (x>=m_properties.gridX || x<0 || y>=m_properties.gridY || y<0)
        return;
...
{//Find n connected color puyo and put coordinates into vector v
    ...
    pos.x=x;
    pos.y=y;
    //v.reserve(m_properties.gridX*m_properties.gridY);
...
{//recursive function that checks neighbours
    ...
    for (k=0;k<4;k++)
    {
        ...
        {
            //fieldPuyoArray[pos.x+hor][pos.y+ver]->mark=true;
            connected++;
...
{//Phase 20: Create puyo and get ready to drop
        ...
        || ((m_player->getPlayerType()==HUMAN && m_player->currentgame->settings->recording==PVS_REPLAYING) && m_player->messages.front()[0]=='p'))
    {
        ...
        //else if (m_player->activeGarbage->GQ<=0 && m_player->feverMode && m_player->feverGauge.seconds!=0)
        //    m_player->forgiveGarbage=true;
...
void field::searchFallDelay() //(deprecated function)
{//set puyo ready to fall and disconnect them
    int count=0;
    int countDelay=0;
    bool foundEmpty=false;
    ...
    for (int i=0;i<m_properties.gridX;i++)
    {
        ...
        for (int j=0;j<m_properties.gridY;j++)
        {
            ...
            {
                fieldPuyoArray[i][j]->fallDelay=countDelay;
                fieldPuyoArray[i][j]->fallFlag=1;
                fieldPuyoArray[i][j]->setFallTarget(count);
                //fieldPuyoArray[i][j]->SetindexY(count); //already handled by drop() function
                count++;
                countDelay++;
...
void field::unsetLinkAll(int i,int j)
    ...
    for (int i=0;i<m_properties.gridX;i++)
    {
        ...
        for (int j=0;j<m_properties.gridY;j++)
        {
            ...
            if (j!=newy)
            {
                ...
                fieldPuyoArray[i][newy]->fallDelay=countDelay;
...
void field::searchBounce(int x,int y,int posy)
{//search bounce strength
    int i,j,funFlag=1,count=1,rememberColor=0;
    for (i=0;i<posy;i++)
    {
        if (isPuyo(x,y-i))
        {
        ...
        if (funFlag==1 && count<5)
        {
            if (isPuyo(x,y-i) && fieldPuyoArray[x][y-i]->fallFlag==0)
            {
                ...
                fieldPuyoArray[x][y-i]->bounceMultiplier=1./pow(2.,count-1);
                fieldPuyoArray[x][y-i]->bounceTimer=2;
            }
            count++;
        }
    }
...
void field::searchLink(int x,int y)
...
void field::loseDrop()
{
    for (int i=0;i<m_properties.gridX;i++)
    {
        for (int j=0;j<m_properties.gridY;j++)
        {
            ...
            {
                unsetLinkAll(i,j);
                //fieldPuyoArray[i][j]->AddAccelY((0.1+int(sqrt(i+2)*12)%10)/4.0);
                //fieldPuyoArray[i][j]->SetSpriteY(fieldPuyoArray[i][j]->GetSpriteY()+(int(sqrt(i+2)*12)%10)/4.0);
                if (fieldPuyoArray[i][j]->GetAccelY()<0.1)
                    fieldPuyoArray[i][j]->SetAccelY((int(sqrt(i+2.)*12)%10)/2.0);
                fieldPuyoArray[i][j]->AddAccelY(0.2);
            }
        }
    }
}

void field::dropField(std::string fieldstring)
{//drop a field of uyos
    int j=0; //j backwards counter
    for (size_t i=0;i<fieldstring.size();i++)
    {
        j=fieldstring.size()-1-i;
        //int x=j%m_properties.gridX;
        int x=m_properties.gridX-1-(i%m_properties.gridX);
        int y=m_properties.gridY-2;
        std::string str=fieldstring.substr(j,1);
        //if (to_int(fieldstring[i])>0 && to_int(fieldstring[i])<6)
        if (to_int(str)>0 && to_int(str)<6)
        {//color puyo
            if (addColorPuyo(x,y,to_int(str)-1,1,i/m_properties.gridX,int(i/m_properties.gridX)))
            {
                //drop after creation
                int newy=dropSingle(x,y);
                //set falltarget
...
void field::setFieldFromString(std::string fieldstring)
{
    //clear field
    clearField();
    ...
    */
    for (size_t i=0;i<fieldstring.size();i++)
    {
        int x=i%m_properties.gridX;
        int y=i/m_properties.gridX;
...
std::string field::getFieldString()
...
void field::test()
{
//    if (m_player->controls.Up>0)
//    {
//        //m_properties.angle=m_properties.angle-1;
//        //m_properties.scaleX=m_properties.scaleX-0.01;
//        //m_properties.scaleY=m_properties.scaleY-0.01;
//        //debugstring=to_string(m_properties.scaleY);
//    }

/*
    debugstring="";
    for (int j=m_properties.gridY-1;j>0;j--)
    {
        for (int i=0;i<m_properties.gridX;i++)
        {
            if (isPuyo(i,j))
            debugstring=debugstring+to_string(fieldPuyoArray[i][j]->getColor()+1)+" ";
            else
